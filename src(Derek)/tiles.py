#%% md
# # TPU Graphs tile baseline: grouped ranking model
# 
# This notebook keeps the simple **graph-summary + XGBoost ranker** baseline, and now evaluates it using the **official tile-side competition metric**.
# 
# For the `tile:xla` collection, the competition uses the **top-K slowdown** of the predicted candidates relative to the true best runtime. In the Google TPU Graphs evaluation script, this is computed as:
# 
# ```python
# error = (best_of_candidates - time_best) / time_best
# ```
# 
# So for a graph:
# 
# - `tile_error@k = (best runtime among predicted top-k - true best runtime) / true best runtime`
# - `tile_score@k = 1 - tile_error@k`
# 
# This notebook is still **tile:xla only**, so it only computes the **tile component** of the full competition metric.  
# It does **not** yet compute the `layout:*` Kendall Tau part.
# 
# It also keeps a few debugging metrics that are still helpful while iterating:
# 
# - exact top-1 hit rate
# - top-5 contains-best rate
# - raw slowdown ratios
# - submission-style `TopConfigs` generation
# - model persistence so you can reuse saved XGBoost models
# 
#%%
import json
...
#%%
# Keep these small for the first run, then increase.
MAX_TRAIN_FILES = 400
MAX_VALID_FILES = 120
...
# -------------------------------------------------------------------
# Saved model / artifact settings
# Bump MODEL_TAG whenever you change the feature pipeline substantially.
# -------------------------------------------------------------------
MODEL_TAG = "tile_xgb_ranker_baseline_v1"
...
def tile_topk_error(y_true_ratio_group, y_pred_score_group, k):
    """
    Official tile-side error for one graph:
        (best runtime among predicted top-k - true best runtime) / true best runtime

    Here `y_true_ratio_group` is already runtime / runtime_normalizer, so lower is better.
    """
...
def tile_topk_score(y_true_ratio_group, y_pred_score_group, k):
    """
    Competition-style tile score component for one graph:
        1 - tile_topk_error
    """
...
display(comparison)
display(valid_submission_preview.head())

#%% md
# ## Notes
# 
# ### What is now “official” in this notebook?
# For this **tile:xla** notebook, the closest validation numbers to the real competition metric are:
# 
# - `tile_error@5`  
# - `tile_score@5 = 1 - tile_error@5`
# 
# Because the tile submission only uses the first **5** predicted configs, `@5` is the main number to watch.
# 
# ### Which one should you optimize?
# - If you want to match the **Google TPU Graphs evaluation script**, watch `tile_error@5` and minimize it.
# - If you want the more **competition-style score view** from the Kaggle description, watch `tile_score@5` and maximize it.
# 
# These are equivalent views of the same thing.
# 
# ### Why keep the other metrics?
# They are still useful for debugging:
# 
# - `top1_exact_hit_rate` tells you how often rank-1 is exactly correct
# - `top5_contains_best_rate` tells you how often the true best config appears inside your top 5
# - slowdown ratios are easier to reason about than abstract scores
# 
# ### Important limitation
# This notebook is still **tile only**.  
# The full competition score also includes the `layout:*` side with **Kendall Tau correlation**, and Kaggle averages the tile and layout components together.
# 
# ### How to avoid retraining every run
# The notebook saves the model to:
# 
# - `MODEL_PATH`
# 
# On the next run, if the file exists and `FORCE_RETRAIN = False`, it will load the saved model directly.
# 
# If you change the feature pipeline substantially, bump `MODEL_TAG` so you do not accidentally reuse an incompatible saved model.
# 